...
from examples.pybullet.utils.pybullet_tools.utils import euler_from_quat, set_camera_pose, draw_aabb, WHITE, get_link_pose, \
...
def load_all_furniture(world, ordering, floor, base, start,
    ...
    def load_furniture(category):
        ...
        return world.add_object(Object(
            ...
            name=category, category=category), Pose(point=Point(2,0,0)))
...
def sample_full_kitchen(world, verbose=True, pause=True, reachability_check=True,
    ...
    cabbage = world.add_object(Movable(
        load_asset('FOOD', yaw=random.uniform(-math.pi, math.pi),
                   floor=floor, random_instance=True, sampling=False),
        category='FOOD'
    ))
    movables = [cabbage]
